...
echo ""
echo "Find the Tensorboard URL:"
echo "  kubectl logs job/$JOB_NAME-worker-0 -c jax-tpu | grep \"Vertex AI Tensorboard\""
echo ""
echo "Exec into the pod for debugging:"
echo "  kubectl exec -it \$(kubectl get pods -l jobset.sigs.k8s.io/jobset-name=$JOB_NAME,batch.kubernetes.io/job-completion-index=0 -o jsonpath='{.items[0].metadata.name}') -- bash"
echo ""
echo "Cleanup:"
