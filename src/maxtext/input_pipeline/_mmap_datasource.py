...
def get_mmap_dataset(
  ...
  """Build an mmap dataset pipeline with optional ``';'``-separated mixture.

  Args:
      apply_transforms: Callback with signature
          ``(dataset, shuffle, seed, epochs, host_idx, host_cnt, threads, prefetch)``
          that applies shuffle / repeat / shard / to_iter.  Typically
          :func:`_apply_mapdataset_transforms`.
  """
  if ";" in data_file_pattern:
    prefixes, weights = _parse_weighted_mixture(data_file_pattern, "mmap")
    datasets = [create_mmap_source(p, split_sentences, seq_length, eod_id) for p in prefixes]
    for i, ds in enumerate(datasets):
      datasets[i] = apply_transforms(
          ds,
          shuffle,
          shuffle_seed,
          num_epoch,
    ...
    return grain.IterDataset.mix(datasets, weights)

  ds = create_mmap_source(data_file_pattern, split_sentences, seq_length, eod_id)
...
def get_mmap_npy_dataset(
  ...
  Args:
      apply_transforms: See :func:`get_mmap_dataset`.
