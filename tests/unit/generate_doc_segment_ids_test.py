...
class TestGenerateDocSegmentIdsResetTrue:
  ...
  def test_multi_doc_with_eod(self):
    ...
    npt.assert_array_equal(result["tokens_segmentation"], [1, 1, 0, 2, 2, 0, 3])
    npt.assert_array_equal(result["tokens_position"], [0, 1, 0, 0, 1, 0, 0])
  ...
  def test_multiple_columns(self):
    ...
    for col in ("input_ids", "target_ids"):
      npt.assert_array_equal(result[f"{col}_segmentation"], [1, 0, 2])
      npt.assert_array_equal(result[f"{col}_position"], [0, 0, 0])
